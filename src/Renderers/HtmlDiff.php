...
class HtmlDiff
{
    ...
    protected function beforeView(string $merged): string
    {
        return new HtmlFragment($merged)
            ...
            ->removeElements('//ins')
            // A wholly new list/table/quote leaves an empty wrapper behind once its rows/items
            // are gone; drop those too, repeating since removing one can empty its own parent.
    ...
    protected function afterView(string $merged): string
    {
        return new HtmlFragment($merged)
            ...
            ->removeElements('//del')
            // A wholly deleted list/table/quote leaves an empty wrapper behind once its
            // rows/items are gone; drop those too.
            ->removeEmptyElements('//ul | //ol | //table | //tbody | //thead | //blockquote | //pre | //code | //mark')
